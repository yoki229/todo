...
class TodoController extends Controller
{
    public function index(){
        ...
        //updateフォームの日付が形式のせいで表示されないのでCarbonの日付型に変換
        $todos-> transform(function ($todo) {
        $todo-> scheduled_date_formatted = $todo->scheduled_date
            ? Carbon::parse($todo->scheduled_date)->format('Y-m-d')
            : '';
            return $todo;
        });
    ...
    public function store(TodoRequest $request){
        $todo = $request->only(['content','category_id','scheduled_date']);
        Todo::create($todo);                        //Todoモデルに沿って、データベースに新しいデータを登録（保存）
        return redirect('/')->with('message','Todoを作成しました');
    }

    public function update(TodoRequest $request){
        $todo = $request->only(['content','scheduled_date']);
    ...
    public function search(Request $request){
        $todos = Todo::with('category')                 //Todoモデルから、すべてのToDoと、レーション先の category（カテゴリ情報）も一緒に読み込む
            ...
            ->DateSearch($request->scheduled_date)      ////scopeの部分を抜いたメソッド名。scheduled_dateで検索
            ->get();                                    //全てを入手
